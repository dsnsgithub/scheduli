...
export function generateICSFile(scheduleDB: UnparsedSchedule): string {
...
END:VTIMEZONE`;
  ...
  const endDate = new Date(
    new Date(scheduleDB.about.endDate).setDate(new Date(scheduleDB.about.endDate).getDate() + 1),
  );
    ...
    for (const dayPattern of routine.days) {
      ...
      for (const date of dates) {
        for (const event of routine.events) {
          ...
          if (isInactiveDay(eventStart, scheduleDB.about.inactiveDays)) continue;
...
function getDatesForPattern(
  dayPattern: UnparsedDay | [UnparsedDay, UnparsedDay] | UnparsedDay[],
  startDate: Date,
  endDate: Date,
): Date[] {
  const dates: Date[] = [];
  ...
  if (typeof dayPattern === "number") {
    ...
    const current = new Date(dayPattern[0]);
...
function isInactiveDay(
  date: Date,
  inactiveDays: UnparsedSchedule["about"]["inactiveDays"],
): boolean {
  return inactiveDays.some((item) => {
