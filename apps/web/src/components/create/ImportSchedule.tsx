...
import { UnparsedSchedule, UnparsedEvent } from "@scheduli/types";
...
export async function convertStudyListToSchedule(
  ...
  const defaultSchedule: UnparsedSchedule = await fetch("/api/schedule/uci").then((response) =>
    response.json(),
  );
        ...
        for (const course of department["courses"]) {
          for (const section of course["sections"]) {
            for (const meeting of section["meetings"]) {
              const courseName = `${course["deptCode"]} ${course["courseNumber"]} ${section["sectionType"]}`;
...
export default function ImportSchedule(props: {
  ...
  const [, setPossibleQuartersData] = React.useState<{ [key: string]: string }[] | null>(null);
  ...
  React.useEffect(() => {
    const fetchQuarters = async () => {
      ...
      console.log(possibleQuartersData ? possibleQuartersData.data[0].shortName : "");
    };
      ...
      <div className="fixed inset-0 flex w-screen items-center justify-center p-10">
        <Dialog.Panel className="w-full max-w-lg bg-wedgewood-200 p-10">
          ...
          <div className="grid justify-items-end">
            <button
              className="bg-wedgewood-400 ml-4 p-3 px-4 rounded-sm mt-6"
              onClick={() => {
            ...
            <textarea
              ...
              className="border-2 border-wedgewood-500 rounded-md px-4 py-2 w-full mt-4"
            ...
            <button
              className="bg-wedgewood-500 hover:bg-wedgewood-600 text-white font-bold py-2 px-4 rounded-sm mt-4"
              onClick={async () => {
