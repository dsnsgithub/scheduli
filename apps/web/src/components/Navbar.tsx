    ...
    <nav className="flex justify-around items-center mt-8 py-4 px-4">
      <Link className="flex items-center" href="/">
        {isLoading ? (
          ...
          <Image
            src={ScheduleIcon(currentDate.getDate())}
            alt="Scheduli Icon"
            width="80"
            quality={100}
          />
      ...
      <div className="lg:hidden">
        <button onClick={toggleMenu} className="p-2 focus:outline-hidden">
          {isOpen ? (
            <FontAwesomeIcon
              icon={faTimes}
              size="2x"
              className="text-wedgewood-600 border border-wedgewood-600 p-2 rounded-lg"
            />
          ) : (
      ...
      {isOpen && (
        <div className="lg:hidden absolute top-0 right-0 mt-16 w-full bg-wedgewood-100 shadow-md p-4 rounded-lg border border-wedgewood-600">
          <div className="flex flex-row-reverse mb-4">
            <button
              onClick={toggleMenu}
              className="focus:outline-hidden text-wedgewood-600 border border-wedgewood-600 rounded-full w-10 h-10 flex items-center justify-center"
            >
          ...
          <div className="flex flex-col items-center">
            <Link
              className="p-4 border-wedgewood-300 bg-wedgewood-200 border-2 rounded-sm w-full text-center m-2 font-bold"
              href="/"
            >
