...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const scheduleDB = JSON.parse(
      fs.readFileSync(path.join(schoolFolder, "/schedule.json"), "utf-8"),
    );

    for (const scheduleName in scheduleDB["routines"]) {
      if (scheduleName == "about") continue;
      const scheduleArray = fs
        .readFileSync(path.join(schoolFolder, `${scheduleName}.txt`), "utf-8")
        .split("\n");
