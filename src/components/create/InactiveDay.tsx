...
function removeInactiveDay(rawName: string, schedule: any, setSchedule: Function) {
	...
	const index = newSchedule["about"]["inactiveDays"].findIndex((item: { days: string | string[] }) => {
...
export default function InactiveDay(props: { rawName: string; schedule: any; description: string; setSchedule: Function; days: string | string[] }) {
	return (
		<div className="shadow-lg p-4 bg-wedgewood-400 lg:ml-4 ml-4 mt-2 flex flex-row items-center justify-center">
			<div>
				...
				<h4 className="text-sm">{typeof props.days == "string" ? props.days : `${props.days[0]} to ${props.days[1]}`}</h4>
			</div>
