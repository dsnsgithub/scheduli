...
export default function EventEditor(props: {
  ...
  for (const index in props.schedule["routines"][props.currentRoutine][
    "events"
  ]) {
    const event =
      props.schedule["routines"][props.currentRoutine]["events"][index];
    eventElemList.push(
      <Event
        key={event["startTime"]}
        name={event["name"]}
  ...
  return (
    <div className="shadow-lg p-2 lg:p-10 bg-wedgewood-200" ref={parent}>
      ...
      <div className="grid justify-items-end">
        <div>
          <button
            className="bg-wedgewood-400 p-4 w-64 mt-6 rounded"
            onClick={() =>
              createNewEvent(
